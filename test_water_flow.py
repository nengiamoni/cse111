...
import pytest
from water_flow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction #psi_to_kpa
...
# I try exceeding the requirement but the code isnt ruuning so i have to comment it 
#def test_psi_to_kpa_conversion():
    #assert psi_to_kpa(0) == approx(0, abs=0.001)
    #assert psi_to_kpa(10) == approx(68.9476, abs=0.001)
    #assert psi_to_kpa(30) == approx(206.8428, abs=0.001)
    #assert psi_to_kpa(50) == approx(344.738, abs=0.001)
